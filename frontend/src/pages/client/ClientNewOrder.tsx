...
export function ClientNewOrder() {
  ...
  const validate = (): boolean => {
    ...
    if (!clientForm.name.trim()) next.clientName = 'Укажите имя';
    if (!clientForm.phone.trim()) next.clientPhone = 'Укажите телефон';

    if (clientForm.email && !/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(clientForm.email)) {
      next.clientEmail = 'Некорректный email';
    }

    if (orderType === 'inspection') {
      if (!inspectionForm.city.trim()) next.inspectionCity = 'Укажите город осмотра';
      if (!inspectionForm.sourceUrl.trim() && (!inspectionForm.make.trim() || !inspectionForm.model.trim())) {
        next.inspectionSource = 'Введите ссылку или Марку и Модель';
      }
    } else {
      if (!selectionForm.city.trim()) next.selectionCity = 'Укажите город клиента';
      if (!selectionForm.cityTarget.trim()) next.selectionTarget = 'Укажите, где ищем';
      if (!selectionForm.budgetMin) next.budgetMin = 'Укажите бюджет';
      if (!selectionForm.budgetMax || selectionForm.budgetMax < selectionForm.budgetMin) {
        next.budgetMax = 'Максимум должен быть ≥ минимума';
      }
    }
    ...
    <div className="page">
      ...
      <form className="section" style={{ gap: 16 }} onSubmit={onSubmit}>
        <div className="form-grid">
          <div className={`field ${errors.clientName ? 'field--error' : ''}`}>
            <label>Имя</label>
            <input value={clientForm.name} onChange={(e) => updateClient('name', e.target.value)} />
            {errors.clientName && <span className="field__error">{errors.clientName}</span>}
          </div>
          <div className={`field ${errors.clientPhone ? 'field--error' : ''}`}>
            <label>Телефон</label>
            <input value={clientForm.phone} onChange={(e) => updateClient('phone', e.target.value)} />
            {errors.clientPhone && <span className="field__error">{errors.clientPhone}</span>}
          </div>
          <div className={`field ${errors.clientEmail ? 'field--error' : ''}`}>
            <label>Email</label>
            <input value={clientForm.email} onChange={(e) => updateClient('email', e.target.value)} />
            {errors.clientEmail && <span className="field__error">{errors.clientEmail}</span>}
          </div>
        </div>
        ...
        {orderType === 'inspection' ? (
          <>
            <div className="form-grid">
              <div className={`field ${errors.inspectionSource ? 'field--error' : ''}`}>
                ...
                {errors.inspectionSource && <span className="field__error">{errors.inspectionSource}</span>}
              </div>
              <div className={`field ${errors.inspectionCity ? 'field--error' : ''}`}>
                <label>Город осмотра</label>
                <input value={inspectionForm.city} onChange={(e) => updateInspection('city', e.target.value)} />
                {errors.inspectionCity && <span className="field__error">{errors.inspectionCity}</span>}
              </div>
            </div>
            <div className="form-grid">
              <div className="field">
          ...
          <>
            <div className="form-grid">
              ...
              <div className={`field ${errors.budgetMax ? 'field--error' : ''}`}>
              ...
              {errors.selectionCity && <span className="field__error">{errors.selectionCity}</span>}
              {errors.selectionTarget && <span className="field__error">{errors.selectionTarget}</span>}
              {errors.budgetMin && <span className="field__error">{errors.budgetMin}</span>}
              {errors.budgetMax && <span className="field__error">{errors.budgetMax}</span>}
              <div className="field">
                <label>Дедлайн</label>
